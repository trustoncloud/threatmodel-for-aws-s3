...
# Helper: Check if Action covers all S3 operations (s3:* or *)
...
result := "skip" {
    ...
    # Fail if no policy exists (Buckets are insecure by default regarding transport enforcement)
    not has_policy
} else := "fail" {
...
# --- Metadata ---
currentConfiguration := "Bucket Policy does not strictly deny insecure transport." {
    result == "fail"
} else := "Bucket Policy explicitly denies insecure transport."

expectedConfiguration := "Bucket Policy must contain a 'Deny' statement for all actions ('s3:*') when 'aws:SecureTransport' is false or 's3:TlsVersion' < 1.2."